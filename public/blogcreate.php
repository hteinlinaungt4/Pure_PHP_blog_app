...
if (isset($_POST['submit'])) {
    ...
    if (empty($filename)) {
        $fileError="You are select is required!";
    }
    if (is_uploaded_file($filetmp)) {
        $newfile=md5($filename) . "." . $fileext;
        $destfile=$filedir .$newfile;
        if (checkMineType($filetmp)) {
            if (!empty($title) && !empty($select) && !empty($content) && !empty($filename)) {
                move_uploaded_file($filetmp,$destfile);
                $query="INSERT INTO blog(image,category,title,content,author) VALUES('$newfile','$select','$title','$content','$author')";
                $result=mysqli_query($db,$query);
                if ($result) {
                    $_SESSION['successMsg']="You are created successfully blog!";
                ...
                <div class="container-fluid">
                    <div class="card">
                        <div class="card-title bg-light d-flex  justify-content-between">
                            <h4 class="p-3 text-primary fw-bold">Blog Creation Form</h4>
                        ...
                        <div class="card-body">
                            <form action="" method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control <?php if(!empty($titleError)) : ?>is-invalid <?php endif ?>" id="title" name="title" value="<?php echo isset($title) ? $title : ''; ?>" >
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="select" class="form-select <?php if(!empty($selectError)) : ?> is-invalid <?php endif ?>" id="title" value="<?php isset($_POST['select']) ? $_POST['select'] : ''; ?>">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" id="textarea" class="form-control <?php if(!empty($contentError)) :?>is-invalid<?php endif ?>" rows="10" ><?php echo isset($content) ? $content : ''; ?></textarea>
